...
regal provides a repository and graph-based api for library data and is developed in the context of edoweb3 - http://www.hbz-nrw.de/angebote/hosting/edoweb/
...
Required: *apache2 with proxy and rewrite module, maven3, git*!

Download "installer.tar.gz":https://raw.github.com/jschnasse/edoweb2/master/installer.tar.gz

Uncompress

bc.. tar -xzf installer.tar.gz
cd installer

p. Edit variables.sh as you need e.g.

bc.. ARCHIVE_HOME=/opt/regal
SERVER=localhost
ARCHIVE_USER=user
...
bc.. chown -R www-data /opt/archive/html

p. Link apache to site.conf

bc.. echo "Include /opt/archive/conf/site.conf" >> /etc/apache2/apache2.conf
...
bc.. cd api
mvn clean install war:war
cp target/api.war $FEDORA_HOME/tomcat/webapps
